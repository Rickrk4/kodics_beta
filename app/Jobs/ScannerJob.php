...
class ScannerJob implements ShouldQueue
{
    ...
    {
        $this->prepareStatus();
        //$this->setInput(['comics_dir_key' => Directory::where('path',config('settings.comics_dir')->first()->id)]);
    ...
    public function handle($dir = null){
        ...
        if(is_dir($dir->path)){
            ...
            foreach($files as $file){
                ...
                if(is_dir($file)){
                    if(!($directory = Directory::where('path',$file)->first())){
                        $directory = Directory::create(['name' =>  pathinfo($file)['basename'], 'path' => $file]);
                        array_push($directories, $directory);
                    }
                    $children = handle($directory);
                    $directory->comics()->saveMany($children['comics']);
                    //$directory->directories()->saveMany($children['directories']);
                    if(!$directory->cover() && ($child = !$directory->comics() ? $directory->directories()->first() : $directory->comics()->first())){
                        $directory->cover_id = $child->cover_id;
                        $directory->save();
                    }
                }else {
